    ...
    <div className="rounded-2xl border border-border-main bg-surface p-6 shadow-sm hover:shadow-md transition-all duration-200 h-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-text-secondary dark:text-text-secondary mt-0.5">Revenue generated per day</p>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between py-2 border-b border-border-main">
          ...
          <span className="text-sm font-bold text-text-main">{conversionRate}%</span>
        </div>
        <div className="flex items-center justify-between py-2 border-b border-border-main">
          <span className="text-xs text-text-secondary">Avg Sales Cycle</span>
          <span className="text-sm font-bold text-text-main">
            {avgSalesCycle > 0 ? `${avgSalesCycle} days` : '—'}
          </span>
        </div>
        <div className="flex items-center justify-between py-2">
          <span className="text-xs text-text-secondary">Formula</span>
          <span className="text-[10px] text-text-secondary italic">(Opp × WR × Deal) ÷ Cycle</span>
        </div>
      </div>
      ...
      <div className={`mt-4 flex items-center gap-2 rounded-xl px-4 py-2.5 ${
        isHealthy
          ...
          : 'bg-background  border border-border-main '
      }`}>
        <span className={`h-2 w-2 rounded-full ${isHealthy ? 'bg-emerald-400 animate-pulse' : 'bg-surface '}`} />
        <span className={`text-xs font-semibold ${isHealthy ? 'text-emerald-600 dark:text-emerald-400' : 'text-text-secondary dark:text-text-secondary'}`}>
