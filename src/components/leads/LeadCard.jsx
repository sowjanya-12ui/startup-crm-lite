...
export default function LeadCard({ lead, onEdit, onDelete }) {
  return (
    ...
    <div className="group relative overflow-hidden rounded-2xl border border-border-main bg-surface p-5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md transition-colors duration-200">

      {/* Top section: name, status badge, and action buttons */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          {/* Lead name */}
          <h3 className="truncate text-base font-bold text-text-main">{lead.name}</h3>
          {/* Company with icon */}
          <div className="mt-1 flex items-center gap-1.5 text-sm text-text-secondary">
            <Briefcase className="h-3.5 w-3.5 shrink-0 text-text-secondary dark:text-text-secondary" />
      ...
      <div className="mt-4 space-y-2">
        ...
        <div className="flex items-center gap-2 text-sm text-text-secondary">
          <Mail className="h-3.5 w-3.5 shrink-0 text-text-secondary dark:text-text-secondary" />
          <span className="truncate">{lead.email}</span>
        </div>
        {/* Phone row (rendered only when phone data exists) */}
        {lead.phone && (
          <div className="flex items-center gap-2 text-sm text-text-secondary">
            <Phone className="h-3.5 w-3.5 shrink-0 text-text-secondary dark:text-text-secondary" />
      ...
      <div className="mt-4 flex items-center gap-2 border-t border-border-main pt-3.5">
        {/* Edit button */}
        <button
          type="button"
          onClick={() => onEdit(lead)}
          className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-border-main bg-background py-2 text-xs font-semibold text-text-secondary transition-all duration-200 hover:border-blue-200 dark:hover:border-blue-800 hover:bg-blue-50 dark:hover:bg-blue-950/40 hover:text-primary dark:hover:text-primary cursor-pointer"
