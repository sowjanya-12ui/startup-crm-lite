...
export default function LeadTable({ leads = [], onEdit, onDelete }) {
  return (
    ...
    <div className="overflow-hidden rounded-2xl border border-border-main bg-surface shadow-sm transition-colors duration-200">
      ...
      <div className="overflow-x-auto">
        ...
        <table className="min-w-full divide-y divide-gray-100 text-left text-sm">
          ...
          {/* Column headers */}
          <thead className="bg-background /50 text-xs font-bold uppercase tracking-wider text-text-secondary">
            <tr>
          ...
          <tbody className="divide-y divide-gray-100">
            {leads.length > 0 ? (
              leads.map((lead) => (
                <tr
                  key={lead._id}
                  className="group transition-colors duration-200 hover:bg-background dark:hover:bg-background"
                >
                  {/* Name cell — with avatar placeholder */}
                  <td className="whitespace-nowrap px-6 py-4">
                    <div className="flex items-center gap-3">
                      {/* Avatar circle */}
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-blue-50 dark:bg-blue-950/40 text-primary dark:text-primary">
                        <User className="h-4.5 w-4.5" />
                      </div>
                      <span className="font-semibold text-text-main">{lead.name}</span>
                    </div>
                  </td>
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-text-secondary">
                    <div className="flex items-center gap-2">
                      <Briefcase className="h-3.5 w-3.5 text-text-secondary dark:text-text-secondary" />
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-text-secondary">
                    <div className="flex items-center gap-1.5">
                      <Mail className="h-3 w-3 text-text-secondary dark:text-text-secondary" />
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      <button
                        type="button"
                        onClick={() => onEdit(lead)}
                        className="rounded-lg p-1.5 text-text-secondary dark:text-text-secondary transition-all duration-200 hover:bg-blue-50 dark:hover:bg-blue-950/40 hover:text-primary dark:hover:text-primary cursor-pointer"
  ...
  className="rounded-lg p-1.5 text-text-secondary dark:text-text-secondary transition-all duration-200 hover:bg-red-50 dark:hover:bg-red-950/40 hover:text-red-600 dark:hover:text-red-400 cursor-pointer"
...
>
              ...
              // Empty state row
              <tr>
                <td colSpan={7} className="px-6 py-16 text-center text-sm text-text-secondary">
